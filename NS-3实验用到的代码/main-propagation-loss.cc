...
std::ofstream plotFile("propagation_loss.plt");
...
int main (int argc, char *argv[])
{
  ...
  GnuplotCollection gnuplots ("main-propagation-loss1.pdf");
...

  {
    ...
    log->SetAttribute ("Exponent", DoubleValue (5));

    Gnuplot plot = TestDeterministic (log);
    plot.SetTitle ("ns3::LogDistancePropagationLossModel (Exponent = 5)");
    gnuplots.AddPlot (plot);
  }
...

  {
    ...
    log3->SetAttribute ("Exponent0", DoubleValue (5.0));
    log3->SetAttribute ("Exponent1", DoubleValue (10.0));
    log3->SetAttribute ("Exponent2", DoubleValue (2.0));

    Gnuplot plot = TestDeterministic (log3);
    plot.SetTitle ("ns3::ThreeLogDistancePropagationLossModel (Exponents 5.0, 10.0 and 2.0)");
    gnuplots.AddPlot (plot);
  }
